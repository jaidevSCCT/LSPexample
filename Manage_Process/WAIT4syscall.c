/*Program to check termination of child using wait4(), the child can be terminated by user only
Reference MAN page of wait4() and for more usage option refer MAN page  */
#include <sys/wait.h>
#include <sys/types.h> 
#include <stdlib.h> 
#include <unistd.h> 
...
int main(int argc, char *argv[]) 
{
	...
	{ /* Code executed by parent */ 
		do 
		{ 
			wpid = wait4(pid, &status, WUNTRACED | WCONTINUED,&usage);
			if (wpid == -1) 
			{ 
				perror("waitpid"); 	
				exit(EXIT_FAILURE); 
